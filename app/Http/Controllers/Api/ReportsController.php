...
class ReportsController extends Controller
{
            ...
            public function dashboard(SalesItem $items)
    {
        ...
        $query->with('Category');
        }))->selectRaw('SUM(quantity) AS total_quantity, item_id')->groupBy('item_id')->orderByRaw('SUM(quantity) DESC')->limit(3)->get();

        $restock = Item::with('Category')->where('quantity', '<=', 3)->orderBy('quantity', 'DESC')->limit(5)->get();
